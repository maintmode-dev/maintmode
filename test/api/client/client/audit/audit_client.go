...
func (a *Client) GetAPIV1AuditLog(params *GetAPIV1AuditLogParams, opts ...ClientOption) (*GetAPIV1AuditLogOK, error) {
	...
	success, ok := result.(*GetAPIV1AuditLogOK)
	if ok {
		return success, nil
	}
	...
	msg := fmt.Sprintf("unexpected success response for GetAPIV1AuditLog: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
