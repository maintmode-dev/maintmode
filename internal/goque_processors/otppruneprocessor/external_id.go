...
// otpPruneExternalID derives the deterministic, day-bucketed external id for an
// otp.prune task. Truncating to the day makes every replica that ticks within
...
// daily-granularity concern, so one prune per day is the intended cadence even
// if the cron schedule fires more often.
//
...
func otpPruneExternalID(now time.Time) string {
	return fmt.Sprintf("otp-prune-%s", now.UTC().Format("2006-01-02"))
}
