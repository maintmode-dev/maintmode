...
// invitationPruneExternalID derives the deterministic, day-bucketed external id
// for an invitation.prune task. Truncating to the day makes every replica that
...
// Retention is a daily-granularity concern, so one prune per day is the intended
// cadence even if the cron schedule fires more often.
//
...
func invitationPruneExternalID(now time.Time) string {
	return fmt.Sprintf("invitation-prune-%s", now.UTC().Format("2006-01-02"))
}
