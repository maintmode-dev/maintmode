...
// xhttp default. The base URL is normalized (trailing slash stripped) so a
...
func NewClient(cfg config.LicenseConfig) *Client {
	return &Client{
		...
		httpc: xhttp.NewClient(
			xhttp.WithTimeout(cmp.Or(cfg.HTTPTimeout, time.Second)),
			xhttp.WithBearerToken(cfg.InstanceToken),
		),
	}
}
